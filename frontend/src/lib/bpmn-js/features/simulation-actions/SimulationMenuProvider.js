 ...
 */
export default function SimulationMenuProvider(
...
SimulationMenuProvider.prototype._register = function() {
	console.log("SimulationMenuProvider._register");
  this._popupMenu.registerProvider('bpmn-simulations', this);
};
 ...
 */
SimulationMenuProvider.prototype.getPopupMenuEntries = function(target) {
  ...
  var filteredReplaceOptions = [];
	...
	var entries =  this._createEntries(target, items.map(x => ({
		label: x.simulationName,
		actionName: `create-simulation-${x.simulationName}`,
		className: 'bpmn-icon-action',
		target: {
			type: 'bpmn:ServiceTask',
			name: x.simulationName,
			data: x,
			event,
		}
	})));

	console.log('getPopupMenuEntries', items, entries);
 ...
 */
SimulationMenuProvider.prototype.getPopupMenuHeaderEntries = function(target) {
 ...
 * @return {PopupMenuEntries}
 */
SimulationMenuProvider.prototype._createEntries = function(target, options) {
  var entries = {};

  var self = this;

  forEach(options, function(option) {
    entries[ option.actionName ] = self._createEntry(option, target);
  });
 ...
 * @return {PopupMenuEntries}
 */
SimulationMenuProvider.prototype._createSequenceFlowEntries = function(target, replaceOptions) {
 ...
 */
SimulationMenuProvider.prototype._createEntry = function(option, target, action) {
  var translate = this._translate;
	var self = this;
  var createAction = function() {
		console.log('createAction', target, option.target);
		var { type, name, event, data } = option.target;
		var newBusinessObject = self._bpmnFactory.create(type);
    var newElement = {
      type: type,
      businessObject: newBusinessObject,
    };
    newElement.di = {};
		newBusinessObject.name = name;

		var shape = self._elementFactory.createShape(newElement);
 ...
 * @return {PopupMenuHeaderEntries}
 */
SimulationMenuProvider.prototype._getLoopCharacteristicsHeaderEntries = function(target) {
 ...
 * @return {PopupMenuHeaderEntries}
 */
SimulationMenuProvider.prototype._getCollectionHeaderEntries = function(target) {
 ...
 * @return {PopupMenuHeaderEntries}
 */
SimulationMenuProvider.prototype._getParticipantMultiplicityHeaderEntries = function(target) {
 ...
 * @return {PopupMenuHeaderEntries}
 */
SimulationMenuProvider.prototype._getAdHocHeaderEntries = function(element) {
  var translate = this._translate;
...
SimulationMenuProvider.prototype._getNonInterruptingHeaderEntries = function(element) {
